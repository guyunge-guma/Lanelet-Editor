...
| 39 | **根因修复**:双 THREE.js 实例导致的所有兼容性问题(pick 失败、dispose 崩溃、事件 handler 异常) | Potree 1.8.2 预构建 `potree.js` 是 webpack 4 打包,THREE.js 被封装在 webpack 模块闭包内,不暴露到 `window`。应用被迫单独加载 `three.min.js`(r124),形成两个 THREE 实例。两个实例的 `Object3D`、`Ray`、`Geometry` 等类不兼容,导致 Potree 内部 handler 处理外部 THREE 创建的对象时崩溃 | ① 创建 `patch_potree.js` 脚本,在 webpack 模块系统中找到 `three.module.js` 模块定义,在模块末尾注入 `window.THREE = __webpack_exports__;`;② Dockerfile 构建时执行补丁脚本;③ `index.html` 删除单独加载 `three.min.js` 的逻辑;④ `MapView.vue` 的 `getTHREE()` 直接读取 `window.THREE`(即 Potree 内部 THREE);⑤ `DrawingManager.ts` 保留 `safeDispose` 等安全方法作为防护网 |
