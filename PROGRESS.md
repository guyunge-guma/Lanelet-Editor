...
| 40 | `dispose()` 崩溃 `Cannot read properties of undefined (reading 'index')`,`addLaneletMesh` 崩溃 `Cannot read properties of undefined (reading 'position')` | DrawingManager 和 Potree viewer 存储在 Vue `ref()` 中,Vue 深度代理了 DrawingManager 内部所有 THREE 对象(geometry/material/mesh)。THREE.js 的事件系统(EventDispatcher)和方法通过 Vue Proxy 访问时 `this` 绑定错乱,导致 `geometry.attributes.position` 返回 undefined、`onGeometryDispose` 找不到 `index` 属性 | `MapView.vue`:① `drawingManagerRef` 改为 `shallowRef`(只跟踪 null/非 null,不深度代理);② 创建 DrawingManager 时用 `markRaw(new DrawingManager(...))` 阻止代理;③ `viewerRef` 同样改为 `shallowRef` + `markRaw(viewer)` |
| 41 | 创建 Lanelet 时崩溃 `Cannot read properties of undefined (reading 'position')` at `addLaneletMesh` | THREE.js r124 中 `ShapeGeometry` 继承旧版 `Geometry`(有 `vertices` 无 `attributes`),`ShapeBufferGeometry` 才继承 `BufferGeometry`(有 `attributes.position`)。代码用了 `new THREE.ShapeGeometry(shape)` 然后访问 `geometry.attributes.position`,因 `attributes` 为 undefined 而崩溃 | 改用 `new THREE.ShapeBufferGeometry(shape)`,`ShapeBufferGeometry` 继承 `BufferGeometry`,有 `attributes.position` |
| 42 | 两个面板线段名称不一致:LineString 面板显示内部 ID(#1, #2)和 "细线/实线",Lanelet 面板显示后端 ID(#1004, #1010)和仅 type 标签 | ① LineStringPanel 的 `lines` 数组用 DrawingManager 内部 ID,LaneletPanel 用后端 ID;② `lineIdMap`(internalId→backendId)只在 MapView 中存在,未共享给 LineStringPanel;③ LaneletPanel 下拉列表只显示 type label,不显示 subtype | ① `MapView.vue`: `lineIdMap` 改为 `reactive(new Map())` 并 `provide('lineIdMap')`;② `LineStringPanel.vue`: `inject('lineIdMap')`,显示时用 `lineIdMap.get(l.id) ?? l.id` 显示后端 ID;③ 提取 `LINESTRING_SUBTYPE_OPTIONS` 和 `lineSubtypeLabel()` 到 `DrawingManager.ts` 供两个面板共用;④ LaneletPanel 下拉列表也显示 subtype(格式 `#1004 (细线 / 实线, 4点)`) |
